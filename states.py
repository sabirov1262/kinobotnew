...
WAITING_MOVIE_SOURCE_MESSAGE_ID = "waiting_movie_source_message_id"
...
WAITING_PREMIUM_SCREENSHOT = "waiting_premium_screenshot"


def set_state(user_id: int, state: str, data: dict = None):
    current_data = user_states.get(user_id, {}).get("data", {}).copy()
    if data:
        current_data.update(data)
    user_states[user_id] = {"state": state, "data": current_data}
